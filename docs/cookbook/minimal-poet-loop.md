...
The smallest working Poet project. Two skills, a backlog, and a config file. This is the "hello world." If you can run this, everything else builds on top of it.
...
`.poet.toml` sets the default model and tells Poet where skills live:
...
`mode = "auto"` means Poet runs the full poet loop unattended. The scheduler reads the backlog, creates orders, spawns agents, merges results, and repeats until there's nothing left to do.
This cookbook sets `mode = "auto"` on purpose for an unattended demo. If you omit `mode`, Poet defaults to `supervised`.
...
Read `brain/todos.md` for backlog items. Write `.poet/orders-next.json`
...
1. [ ] Add a hello-world CLI command that prints "Hello from Poet"
2. [ ] Write a README explaining what this project does
```

Numbered checkboxes. The scheduler reads these, creates one order per unchecked item, and the poet loop works through them. As items complete, they get checked off.
...
1. Poet reads `.poet.toml` and discovers the two skills.
2. The scheduler runs first. It reads `brain/todos.md`, finds two unchecked items, and writes orders to `.poet/orders-next.json`.
3. Poet picks up the first order and spawns an agent as a child process with its own worktree.
...
6. The poet loop re-schedules. The scheduler sees one item is done, creates orders for remaining work.
7. This continues until all backlog items are checked off.

Open `localhost:3000` to watch it in the web UI, or run `poet status` from the terminal.
