...
Adapters bridge external systems into Poet's backlog. If your team tracks work in GitHub Issues, Linear, Jira, or somewhere else, an adapter syncs those items so Poet can schedule and execute them.
...
An adapter is a set of commands configured in `.poet.toml`. Each command handles one operation:

- **sync**: pulls items from the external source into Poet's backlog format
...
| `status` | string | no       | used by Poet to filter done items |
| `plan`   | string | no       | used by Poet to read plan files  |

`status` and `plan` are the only optional fields Poet interprets. All other fields (e.g. `tags`, `section`, `estimate`, `priority`, or anything your system tracks) are passed through as-is. The scheduler sees them in mise.json and can use them for context, but Poet itself doesn't act on them.

### `done`

Poet calls your `done` command when an order completes, passing the item ID as the first argument. Your command should mark that item complete in your external system. No stdin, no output expected.
...
Not yet called by the poet loop. When implemented, your `add` command will receive JSON on stdin and should create the item in your external system, then print the new item's ID to stdout.

### `edit` _(planned)_

Not yet called by the poet loop. When implemented, your `edit` command will receive the item ID as the first argument and JSON on stdin with the new values.

## Validation

Poet validates each NDJSON line from the sync command individually. Invalid items are skipped — they don't block the rest of the backlog from processing.
