...
A runtime is where an agent session executes. Poet ships with two: local processes and [Sprites](https://sprites.dev) cloud sandboxes. The scheduling agent can pick a runtime per stage, or you can set a project-wide default. Custom runtime plugins are on the roadmap, so you'll be able to define your own.
...
Process is always available. Sprites requires a valid token in the configured environment variable. Poet checks for this at startup and only advertises runtimes that have working credentials.
...
Custom runtime plugins are coming. You'll be able to add your own VM or execution environment by implementing Poet's runtime interface. If you run your own infrastructure and the built-in runtimes don't fit, this will let you plug it in directly.
