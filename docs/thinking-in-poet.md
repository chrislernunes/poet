...
The [Introduction](/introduction) and [Getting Started](/getting-started) cover what Poet does. This page is about how to think when using it.
...
With traditional tools, you'd wire skills together: review calls deploy, deploy calls notify. In Poet, each skill describes when it should run, and the scheduler figures out the order.
...
That's a working poet loop. Run it. Watch what happens. You'll quickly feel what's missing. Agents commit with bad messages? Write a `commit` skill. You're reviewing every PR yourself? Write a `review` skill and schedule it. Agents keep making the same mistakes? Add a `reflect` skill.
...
- [Getting Started](/getting-started) — install Poet and get a loop running
- [Skills](/concepts/skills) — scheduled vs general skills, discovery, composition
- [Scheduling](/concepts/scheduling) — how the poet loop works under the hood
