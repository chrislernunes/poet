...
Poet is configured through a `.poet.toml` file at the root of your project. When no config file exists, Poet uses sensible defaults for all values.
...
Configures where Poet discovers skill definitions.
...
Adapters bridge external systems (issue trackers, project boards) into Poet's backlog. Each adapter is a named table mapping a skill to a set of shell scripts.
